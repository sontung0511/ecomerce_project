...
#             cursor.execute(
#                 'SELECT * FROM products WHERE  pName = % s', (name_product,)) 
...
#                 cursor.execute(
#                     'INSERT INTO carts VALUE (Null, % s, % s, % s)',
...
#         cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
#         cursor.execute(
#             'SELECT * FROM carts WHERE customer_id = % s', (9,))
#         data = cursor.fetchall()
#         if data:
#             for row in data:
#                 cursor.execute(
#                 'SELECT * FROM products WHERE id = % s', (row["product_id"],))
#                 product = cursor.fetchone()
#                 total += int(product["price"]) * row["quantity"]
#         status = True
